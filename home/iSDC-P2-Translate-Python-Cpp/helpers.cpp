...
// 	vector < vector <float> > newGrid;
  	...
  	float centre_prob=1.0-blurring;
  	...
  	row.clear();
  	row.push_back(corner_prob);
  	row.push_back(adjacent_prob);
	row.push_back(corner_prob);
  	window.push_back(row);
  
  	row.clear();
  	row.push_back(adjacent_prob);
  	row.push_back(centre_prob);
	row.push_back(adjacent_prob);
  	window.push_back(row);
	...
	row.push_back(corner_prob);
  	...
  	vector< vector<float> > newGrid(r,vector <float> (c,0.0));
  	float grid_val;
  	float mult;
  	for(int i=0;i<r;i++)
    {
      for(int j=0;j<c;j++)
      {
        ...
        for(int dx=-1;dx<2;dx++)
        {
          for(int dy=-1;dy<2;dy++)
          {
            ...
            int new_i=((i+dy)%r+r)%r;
            int new_j=((j+dx)%c+c)%c;
